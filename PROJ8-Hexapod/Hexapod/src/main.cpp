...
void
gait_ripple(int reverse, int hipforward, int hipbackward, int kneeup, int kneedown, long timeperiod, int leanangle) {
...
void setServo(int servonum, int position) {
    ...
    if (TrimInEffect && servonum < 12) {
        ...
        p += ServoTrim[servonum] -
             TRIM_ZERO;   // adjust microseconds by trim value which is renormalized to the range -127 to 128
    }
...
unsigned int readUltrasonic() {  // returns number of centimeters from ultrasonic rangefinder
    ...
    unsigned int duration = pulseIn(ULTRAINPUTPIN, HIGH,
                                    18000);  // maximum 18 milliseconds which would be about 10 feet distance from object
    ...
    return (duration) /
           58;  // this converts microseconds of sound travel time to centimeters. Remember the sound has to go back and forth
...
Serial.println("");
        ...
        switch (packetState) {
            case P_WAITING_FOR_HEADER:
                if (c == 'V') {
                    ...
                    //Serial.print("GOT V ");
                           ...
                           '@') {  // simplified mode, makes it easier for people to write simple apps to control the robot
            ...
            case P_WAITING_FOR_VERSION:
                ...
                break;
            case P_WAITING_FOR_LENGTH: { // need scope for local variables
                packetLength = c;
            ...
            case P_SIMPLE_WAITING_FOR_DATA:
                ...
                if (packetLengthReceived == 3) {
                    ...
                    if ((packetData[0] != 'W' && packetData[0] != 'D' && packetData[0] != 'F') ||
                        (packetData[1] != '1' && packetData[1] != '2' && packetData[1] != '3' &&
                         packetData[1] != '4') ||
...
void gait_command(int gaittype, int reverse, int hipforward, int hipbackward, int kneeup, int kneedown, int leanangle,
                  int timeperiod) {
...
#define RAWSERVODETACH 254
            ...
            case 'G': // Gait command (coming from Scratch most likely). This command is always 10 bytes long
                ...
                if (i <= packetLengthReceived - 10) {
                    ...
                    LastGleanangle = constrain(lean - 70, -70,
                                               70);  // lean comes in from 0 to 60, but we need to bring it down to the range -30 to 30

                    gait_command(LastGgaittype, LastGreverse, LastGhipforward, LastGhipbackward, LastGkneeup,
                                 LastGkneedown, LastGleanangle, LastGtimeperiod);
            ...
            case 'T': // Trim command
                ...
                if (i <= packetLengthReceived - 2) {
                    ...
                    switch (command) {
                        case 'f':
                        case 'b':
                            ServoTrim[TrimCurLeg + NUM_LEGS] = constrain(
                                    ServoTrim[TrimCurLeg + NUM_LEGS] + ((command == 'b') ? -1 : 1), 0, 255);
                            beep(300, 30);
                            break;

                        case 'l':
                        case 'r':
                            ServoTrim[TrimCurLeg] = constrain(ServoTrim[TrimCurLeg] + ((command == 'r') ? -1 : 1), 0,
                                                              255);
                            beep(500, 30);
                            break;
...
void checkForServoSleep() {
    ...
    if (millis() > freqWatchDog) {
        ...
        Wire.write(
                0);  // address 0 is the MODE1 location of the servo driver, see documentation on the PCA9685 chip for more info
        ...
        if (mode1 & 16) { // the fifth bit up from the bottom is 1 if controller was asleep
            ...
            SuppressScamperUntil = millis() +
                                   10000;  // no scamper for you! (for 10 seconds because we ran out of power, give the battery
